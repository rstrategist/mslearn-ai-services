"""
This Flask web application interfaces with Azure Cognitive Search to provide search functionality
for Margie's Travel documents. It allows users to search through indexed documents, filter results
by metadata, and sort results based on various criteria. The application uses environment variables
to configure the Azure Search service endpoint, query key, and index name.
"""
...
def search_query(search_text, filter_by=None, sort_order=None):
    try:
        ...
        # Submit search query
        # Search mode requires all words in the search text are found in the document
        # Results are filtered to include only documents that match the provided filter expression.
        # Results are ordered by the specified sort expression.
        # Facets are requested for the metadata_author field.
        # Highlighting is requested for the merged_content and imageCaption fields.
...
def search():
    try:
        ...
        return render_template(
            "search.html", search_results=results, search_terms=search_text
        )
